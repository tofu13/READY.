...
from .constants import BITRANGE, CLOCK_PER_FRAME, COLORS, PALETTE, VIDEO_SIZE
...
class TextScreen(VIC_II):
    ...
    def clock(self, clock_counter: int) -> pygame.Surface:
        if clock_counter % CLOCK_PER_FRAME == 0:  # Screen on - no sprites
...
class NumpyScreen(VIC_II):
    ...
    def clock(self, clock_counter: int) -> np.array:
        if clock_counter % CLOCK_PER_FRAME == 0:
